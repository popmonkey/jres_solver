...
def process_driver_results(res, data, total_stints, stint_laps, driver_pool):
    ...
    for d in range(len(driver_pool)):
        for s in range(total_stints):
            if solution[d * total_stints + s] > 0.5:
...
def generate_spotter_schedule(driver_schedule, data, stint_with_pit_seconds):
    ...
    for s, driver_name in enumerate(driver_schedule):
        ...
        for spotter in spotter_pool:
            ...
            if spotter_name == driver_name: continue
            if data['availability'][spotter_name].get(availability_key, 'Unavailable') == 'Unavailable': continue
    ...
    for entry in schedule:
        ...
        if driver_name in member_schedules:
            for i in range(start_hour_index, end_hour_index + 1):
                if i < race_duration_hours: member_schedules[driver_name]['schedule'][i] = "Driving"
    ...
    for name, duties in raw_duties.items():
        ...
        for duty in consolidated_duties:
            ...
            if (duty_start_local - last_duty_end_local).total_seconds() > 60:
                final_itineraries[name].append({"start_local": last_duty_end_local, "end_local": duty_start_local, "activity": "Resting", "duration": duty_start_local - last_duty_end_local})
            final_itineraries[name].append({"start_local": duty_start_local, "end_local": duty_end_local, "activity": duty['activity'], "duration": duty_end_local - duty_start_local})
        ...
        if (race_end_local - last_duty_end_local).total_seconds() > 60:
            final_itineraries[name].append({"start_local": last_duty_end_local, "end_local": race_end_local, "activity": "Resting", "duration": race_end_local - last_duty_end_local})
