...
1.  **`solver.py`**: A powerful command-line tool that ingests race and team data from a JSON input to compute an optimal, fair, and balanced schedule. It considers driver availability, stint limits, and rest requirements to create the schedule.
2.  **`reporter.py`**: A tool that takes the schedule generated by the solver and produces user-friendly reports in multiple formats, including detailed per-member itineraries in their local timezones.
...
```bash
python reporter.py solved_schedule.json --output race_report.xlsx --format xlsx
```
