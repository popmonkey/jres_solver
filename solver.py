...
def print_human_readable_schedule(data, schedule, stint_laps):
    ...
    print("--- 🏁 Race Schedule ---")
    print("="*80)
    print(f"{'Stint':<7} | {'Start Time (UTC)':<25} | {'End Time (UTC)':<25} | {'Driver':<15}")
    print("-" * 80)
    ...
    # --- Loop through schedule and print each stint ---
    for assignment in schedule:
        stint_num = assignment['stint']
        driver = assignment['driver']

        # Calculate start and end times for the current stint
        ...
        print(f"{stint_num:<7} | {start_utc_str:<25} | {end_utc_str:<25} | {driver:<15}")
...
def main():
    ...
    if schedule_assignments:
        # Always print the human-readable schedule to the console
        print_human_readable_schedule(data, schedule_assignments, stint_laps)

        # If an output path is given, also save the JSON file
        if args.output:
            output_data = {
                "raceData": data,
